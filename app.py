...
# Garde-fou : app.py et kobo_utils.py doivent être de la même génération
VERSION_REQUISE = "2026-09-07"
_manquant = [f for f in ("trouver_xlsform", "dataframe_vide", "FORM_QUANTI", "FORM_QUALI")
             if not hasattr(ku, f)]
if _manquant or getattr(ku, "VERSION", "") != VERSION_REQUISE:
    st.error(
        f"**`kobo_utils.py` n'est pas à jour** (version détectée : "
        f"`{getattr(ku, 'VERSION', 'inconnue')}`, attendue : `{VERSION_REQUISE}`)."
    )
    st.info("Déposez la dernière version de **`kobo_utils.py`** à côté de `app.py` "
            "(ainsi que `form_quanti.xlsx`, `form_quali.xlsx` et `logo_sgp.jpeg`), "
